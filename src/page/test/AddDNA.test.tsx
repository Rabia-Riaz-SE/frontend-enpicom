...
describe('AddDNA Component with Axios', () => {
  it('should call axios to save DNA', async () => {
    ...
    // Simulate user input
    fireEvent.change(screen.getByPlaceholderText('Enter DNA'), { target: { value: 'ACTG' } });
    ...
    await waitFor(() => {
      expect(axios.post).toHaveBeenCalledWith(expect.any(String), { DNA: 'ACTG' });
    });
  });

  it('should handle axios error', async () => {
    const mockError = { response: { data: { message: 'Response Error Adding DNA.' } } };
    (axios.post as jest.Mock).mockRejectedValue(mockError);

    render(<AddDNA />);
    fireEvent.change(screen.getByPlaceholderText('Enter DNA'), { target: { value: 'q' } });
