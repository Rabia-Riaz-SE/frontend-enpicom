...
describe('SearchDNA Component', () => {
  it('renders the SearchDNA component', () => {
    ...
    render(<SearchDNA />);
    const inputElement = screen.getByPlaceholderText('Enter DNA sequence to search');
  ...
  it('calls axios and updates the DNA list when button is clicked', async () => {
    ...
    await act(async () => {
      fireEvent.change(screen.getByPlaceholderText('Enter DNA sequence to search'), { target: { value: 'ACTG' } });
  ...
  it('shows error message on axios failure', async () => {
    const mockError = { response: { data: { message: 'Error Fetching DNA Records.' } } };
